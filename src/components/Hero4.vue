<template>
    <ul
        class="grid grid-cols-1 grid-rows-none gap-4 overflow-auto xl:max-h-[56rem] xl:grid-rows-2 lg:gap-4 md:grid-cols-12 md:grid-rows-3" style="padding-left: 10%; padding-right: 10%;">
        <li v-for="item in gridItems" :key="item.title" :class="cn('min-h-[14rem] list-none', item.area)">
                ...
                <div
                    ...
                    <div class="relative flex flex-1 flex-col justify-between gap-3">
                        <div class="w-fit rounded-lg border border-gray-600 p-2">
                            <component :is="item.icon" class="size-4 text-black dark:text-neutral-500" />
                        </div>
                        <div class="space-y-3">
                            <h3
...
import { BoxIcon, SettingsIcon, MusicIcon, SparkleIcon, SearchIcon } from 'lucide-vue-next';
